...
function ModLoader.MergeManifest(into, from)
	...
	for _,k in ipairs{"EntityTypes", "EffectTypes", "TaskLists", "Technologies", "Models", "Animations", "AnimSets", "SelectionTextures_Add",
			"SelectionTextures_Reload", "GUITextures_Add", "GUITextures_Reload", "TerrainTextures_Add", "TerrainTextures_Reload", "WaterTypes",
				"TerrainTypes", "DirectXEffects", "ArmorClasses", "EntityCategories"} do
		local at = into[k]
...
function ModLoader.CleanupMods(modlist, script)
	for _,m in ipairs(modlist.Mods) do
		if not m.KeepArchive and (script or m.ScriptPath == "") then
			m.Archive:Remove()
			m.Archive = nil
		end
	end
...
function ModLoader.LoadModScripts()
	...
	ModLoader.CleanupMods(ModLoader.ModList.Mods, true)
end
