...
public:
	virtual bool MatchesEntity(EGL::CGLEEntity* e, float* rangeOut, int* prio) const = 0;
};
...
public:
	...
	void Reset();
	EGL::CGLEEntity* GetNext(float* rangeOut, int* prio);
	EGL::CGLEEntity* GetNearest(float* rangeOut);
	EGL::CGLEEntity* GetFurthest(float* rangeOut);

	EGL::CGLEEntity* GetNext(int& icurr, float* rangeOut, int* prio) const;
	...
	public:
		EGL::CGLEEntity* operator*() const;
		bool operator==(const Iter& o) const;
		bool operator!=(const Iter& o) const;
		Iter& operator++();
		Iter operator++(int);
		Iter(const EntityIterator& i, int c, EGL::CGLEEntity* e);
	};
...
private:
	int type;
public:
	virtual bool MatchesEntity(EGL::CGLEEntity* e, float* rangeOut, int* prio) const;
...
public:
	...
	static bool IsNotFleeingFrom(EGL::CGLEEntity* e, shok::Position Center, float Range);
};

struct EntityIteratorPredicateFunc : EntityIteratorPredicate {
private:
	std::function<bool(EGL::CGLEEntity* e)> func;
public:
	virtual bool MatchesEntity(EGL::CGLEEntity* e, float* rangeOut, int* prio) const;
	EntityIteratorPredicateFunc(std::function<bool(EGL::CGLEEntity* e)> f);
};
