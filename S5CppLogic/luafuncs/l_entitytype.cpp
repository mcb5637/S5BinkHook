...
namespace CppLogic::EntityType {
	namespace {
		...
		int RemoveEntityCategory(luaext::State L) {
			...
			auto cat = static_cast<shok::EntityCategory>(L.CheckInt(2));
			auto v = t->LogicProps->Categories.SaveVector();
			auto e = std::remove_if(v.Vector.begin(), v.Vector.end(), [cat](shok::EntityCategory c) {
				return c == cat;
			});
		...
		int GetUpgradeCategory(luaext::State L) {
			auto ucat =
				(*GGL::CGLGameLogic::GlobalObj)->GetPlayer(shok::PlayerId::P1)->SettlerUpgradeManager->GetUpgradeCategoryOfEntityType(L.CheckEnum<shok::EntityTypeId>(1));
			L.Push(ucat);
			return 1;
		}
		...
		auto WorkerGetRefinerData(GGlue::CGlueEntityProps* t) {
			auto* p = t->GetBehaviorProps<GGL::CWorkerBehaviorProps>();
			if (p == nullptr)
				throw lua::LuaException("no worker behavior props");
			return std::tuple{p->ResourceToRefine, p->TransportAmount};
		}
		...
		constexpr std::array Settler{
			...
			luaext::FuncReference::GetRef<WorkerGetRefinerData>("WorkerGetRefinerData"),
		};
