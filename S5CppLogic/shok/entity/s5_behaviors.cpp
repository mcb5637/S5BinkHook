...
void GGL::CCamouflageBehavior::HookOnAttacked()
{
	...
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x5011DF), CppLogic::Hooks::MemberFuncPointerToVoid(&CCamouflageBehavior::EventOverrideOnAttacked, 0), reinterpret_cast<void*>(0x5011E6));
}
...
void GGL::CCamouflageBehavior::HookActivate()
{
	...
	void* t = CppLogic::Hooks::MemberFuncPointerToVoid(&CCamouflageBehavior::ActivateCamoOverride, 0);
...
void __declspec(naked) bombattachment_fix() {
	__asm {
		...
		mov ecx, eax;
		call EGL::CGLEEntity::AttachEntityStatic;


		or dword ptr [ebp - 4], 0x0FFFFFFFF;
		lea ecx, [ebp - 0x3C];
...
void GGL::CRangedEffectAbility::HookHealAffected(bool active)
{
	...
	if (active)
		CppLogic::Hooks::RedirectCall(reinterpret_cast<void*>(0x4E3C78), CppLogic::Hooks::MemberFuncPointerToVoid(&CRangedEffectAbility::AdvHealAffected, 0));
	else
...
void __fastcall GGL::CConvertSettlerAbility::PerformConversionStatic(CConvertSettlerAbility *th) {
	th->PerformConversion();
}

void __declspec(naked) convertsettler_hookasm() {
	__asm {
		mov ecx, edi;
		call GGL::CConvertSettlerAbility::PerformConversionStatic;
...
void GGL::CBattleBehavior::HookDamageOverride()
{
	...
	// event
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x50C77B), CppLogic::Hooks::MemberFuncPointerToVoid(&CBattleBehavior::EventOverrideGetDamage, 0), reinterpret_cast<void*>(0x50C788));
	// task hurt
...
void __thiscall GGL::CGLBehaviorAnimationEx::AddNonCancelableHandlers(CGLBehaviorAnimationEx* th)
{
	EGL::CGLEEntity* e = EGL::CGLEEntity::GetEntityByID(th->EntityId);
	e->CreateTaskHandler<shok::Task::TASK_WAIT_FOR_ANIM_NON_CANCELABLE>(th, &CGLBehaviorAnimationEx::TaskWaitForAnimNonCancelable);
...
void GGL::CGLBehaviorAnimationEx::HookNonCancelableAnim()
{
	...
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x588408), &AddNonCancelableHandlersASM, reinterpret_cast<void*>(0x588410));
}
...
void GGL::CWorkerBehavior::HookSupplierSkip()
{
	...
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x4D1689), &AddSupplierSkipASM, reinterpret_cast<void*>(0x4D168F));
}

int __thiscall GGL::CWorkerBehavior::DoWorkEvents(CWorkerBehavior* th, GGL::CBuilding* b, EGL::CGLETaskArgs* t)
{
	if (t->TaskType == shok::Task::TASK_DO_RESEARCH_STEP) {
		EGL::CEvent1Entity ev{ shok::EventIDs::University_ResearchStep, th->EntityId };
		b->FireEvent(&ev);
		return 0;
	}
...
void __thiscall GGL::CWorkerBehavior::AddSupplierSkip(CWorkerBehavior* th)
{
	auto* e = EGL::CGLEEntity::GetEntityByID(th->EntityId);
	e->CreateTaskHandler<shok::Task::TASK_SKIP_SUPPLIER_IF_RESEARCHING>(th, &CWorkerBehavior::TaskSkipSupplierIfResearching);
	e->CreateTaskHandler<shok::Task::TASK_SKIP_SUPPLIER_IF_RESEARCHING_TARGET>(th, &CWorkerBehavior::TaskSkipSupplierIfResearchingTarget);
	e->CreateTaskHandler<shok::Task::TASK_REFINER_RESET_CARRIED_RESOURES>(th, &CWorkerBehavior::TaskResetCarriedResources);
	e->CreateTaskHandler<shok::Task::TASK_REFINER_CHECK_NEEDS_RESOURCES>(th, &CWorkerBehavior::TaskCheckNeedsRes);
}
...
float __thiscall GGL::CAutoCannonBehavior::GetMaxRangeBaseStatic(const CAutoCannonBehavior *th) {
...
void GGL::CAutoCannonBehavior::HookDamageOverride()
{
	...
	// get damage event
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x50F5ED), CppLogic::Hooks::MemberFuncPointerToVoid(&CAutoCannonBehavior::EventGetDamageOverride, 0), reinterpret_cast<void*>(0x50F5F3));
	// fire projectile (also used by trap)
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x510638), CppLogic::Hooks::MemberFuncPointerToVoid(&CAutoCannonBehavior::TaskFireProjectileOverride, 0), reinterpret_cast<void*>(0x510647));
...
void __declspec(naked) autocannonmaxrangeasm() {
	__asm {
		...
		pushad;
		call GGL::CAutoCannonBehavior::GetMaxRangeBaseStatic;
		popad;
...
void GGL::CSerfBehavior::HookMineTrigger()
{
	...
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x4DEAE2), &TaskExtractAdditionalASM, reinterpret_cast<void*>(0x4DEAE7));
}
...
{
	static bool HookBuyTriggers_Hooked = false;
	...
	CFoundryBehavior::HookBuyTrigger();
}
...
void GGL::CFoundryBehavior::HookBuyTrigger() {
	static bool Hooked = false;
	if (Hooked)
		return;
	Hooked = true;
	CppLogic::Hooks::SaveVirtualProtect vp{0x10, {
		reinterpret_cast<void*>(0x50dedb),
	}};
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x50dedb), CppLogic::Hooks::MemberFuncPointerToVoid(&CFoundryBehavior::EventBuyCannonOverride, 0), reinterpret_cast<void*>(0x50dee5));
}

void GGL::CFoundryBehavior::EventBuyCannonOverride(EGL::CEventValue_Int* ev) {
	if (CannonType != shok::EntityTypeId::Invalid)
		return;
	auto* b = static_cast<GGL::CBuilding*>(EGL::CGLEEntity::GetEntityByID(EntityId)); // NOLINT(*-pro-type-static-cast-downcast)
	if (!b->IsIdle(true, true))
		return;
	auto* pl = (*GGL::CGLGameLogic::GlobalObj)->GetPlayer(b->PlayerId);
	auto etyid = static_cast<shok::EntityTypeId>(ev->Data);
	auto* ety = CppLogic::GetEntityType(etyid);
	auto& cost = static_cast<GGL::CGLSettlerProps*>(ety->LogicProps)->Cost; // NOLINT(*-pro-type-static-cast-downcast)
	CppLogic::Events::CanBuySettlerEvent cev{ shok::EventIDs::CppLogicEvent_CanBuySettler, EntityId, shok::EntityId::Invalid, etyid,
		pl->PlayerAttractionHandler->AIPlayerFlag || (pl->PlayerAttractionHandler->GetAttractionUsage() < pl->PlayerAttractionHandler->GetAttractionLimit()),
		pl->CurrentResources.HasResources(&cost),
		!pl->PlayerAttractionHandler->IsMotivationLocked(), !b->WorkerAlarmModeActive };
	(*EScr::CScriptTriggerSystem::GlobalObj)->RunTrigger(&cev);
	if (!cev.Cost) {
		EGL::CNetEventPlayerID f{shok::FeedbackEventIds::FEEDBACK_EVENT_NO_MONEY, b->PlayerId};
		EGUIX::FeedbackEventHandler::GlobalObj()->FireEvent(&f);
		return;
	}
	if (cev.Cost && cev.VCPop && cev.Alarm && cev.Motivation) {
		pl->CurrentResources.SubResources(cost);
		CannonType = etyid;
		CannonProgress = 0;
		EGL::CEvent1Entity e{shok::EventIDs::Foundry_BuildCannonCommand, EntityId};
...
{
	auto* defbuild_gettar = reinterpret_cast<EGL::CGLEEntity * (__thiscall*)(GGL::CDefendableBuildingBehavior*, const shok::Position*)>(0x4FC22F);
...
int __thiscall GGL::CMineBehavior::TaskMineAdd(CMineBehavior* th, int* am, GGL::CResourceDoodad* d, BB::CEvent* ev)
{
...
void GGL::CMineBehavior::HookMineTrigger()
{
	...
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x4E6961), &TaskMineAddASM, reinterpret_cast<void*>(0x4E6966));
}
...
void __thiscall GGL::CKeepBehavior::EventBuySerfOverride(BB::CEvent* ev)
{
	...
	{
		auto* create = reinterpret_cast<GGL::CSettler * (__thiscall*)(GGL::CKeepBehavior*)>(0x4F24A0);;
		if (auto* serf = create(this)) {
			serf->Vanish();
			...
			pl->CurrentResources.SubResources(cost);
			EGL::CEvent2Entities e{shok::EventIDs::Keep_BuySerfCommand, EntityId, serf->EntityId};
			(*EScr::CScriptTriggerSystem::GlobalObj)->RunTrigger(&e);
		}
	}
}
