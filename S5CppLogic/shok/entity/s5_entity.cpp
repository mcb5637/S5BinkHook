...
{
	shok::Position p = Position; // NOLINT(*-slicing)
...
void __thiscall EGL::CGLEEntity::AttachEntityStatic(EGL::CGLEEntity *th, shok::AttachmentType attachtype,
	shok::EntityId otherId, shok::EventIDs eventIdOnThisDetach, shok::EventIDs eventIdOnOtherDetach) {
	th->AttachEntity(attachtype, otherId, eventIdOnThisDetach, eventIdOnOtherDetach);
}
...
void __thiscall GGL::CEntityProfile::EntityRef::AssignAdvExperienceClass(EntityRef* th)
{
...
void GGL::CEntityProfile::HookExperienceClassAssignment(bool active) {
	...
	if (active)
		CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x4C71CA), &AssignAdvExperienceClassASM, reinterpret_cast<void*>(0x4C71D0), entityprofile_experienceclassassignmenthook_backup);
...
bool GGL::CSettler::IsIdle()
{
	...
	return com == shok::LeaderCommand::Defend || com == shok::LeaderCommand::HoldPos || CurrentTaskListID == static_cast<GGL::CGLSettlerProps*>(GetEntityType()->LogicProps)->IdleTaskList; // NOLINT(*-pro-type-static-cast-downcast)
}
...
void GGL::CResourceDoodad::HookAutoDestroyIfEmpty()
{
	...
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x4B8662), &OnEmptyDestroyASM, reinterpret_cast<void*>(0x4B8668));
}
void __thiscall GGL::CResourceDoodad::OnEmptyDestroy(CResourceDoodad* th)
{
	if (th->IsEntityInCategory(RefillableCategory))
		return;
	th->Destroy();
}
...
int GGL::CBuilding::GetNearestFreeConstructionSlotFor(shok::Position* p) const
{
...
bool GGL::CBuilding::IsIdle(bool forRecruitemnt, bool ignoreAlarm)
{
	...
	if (GetTechnologyInResearch() != shok::TechnologyId::Invalid)
		return false;
	{
		auto* f = GetBehavior<GGL::CFoundryBehavior>();
...
{
	const auto entitysetscriptname = reinterpret_cast<void(__thiscall* const)(EGL::CGLEEntity*, const char*)>(0x57A728);
...
EGL::CGLEEntity* EGL::CGLEEntity::AdvChangePlayer(shok::PlayerId player)
{
	...
	if (ScriptName) {
		...
		c.ScriptName = static_cast<char *>(shok::Malloc(sizeof(char) * len));
	...
	if (auto* lb = GetBehaviorDynamic<GGL::CLeaderBehavior>()) {
		...
		auto* settler = static_cast<GGL::CSettler*>(ne); // NOLINT(*-pro-type-static-cast-downcast)
		for (shok::EntityId i : sol) {
			settler->LeaderAttachSoldier(EGL::CGLEEntity::GetEntityByID(i)->AdvChangePlayer(player)->EntityId);
		}
		auto* nlb = settler->GetBehaviorDynamic<GGL::CLeaderBehavior>();
	...
	if (auto* rep = GetBehavior<GGL::CReplaceableEntityBehavior>()) {
...
void EGL::CGLEEntity::AdvancedHurtEntityBy(CGLEEntity* attacker, int damage, shok::PlayerId attackerFallback, bool uiFeedback, bool xp, bool addStat, shok::AdvancedDealDamageSource sourceInfo)
{
	...
	if (attacker && !attacker->EventIsInvisible()) {
		if (!EventIsWorker() || !(*CGLEGameLogic::GlobalObj)->Landscape->IsPosBlockedInMode(&Position, CGLELandscape::BlockingMode::Blocked)) {
	...
	if (uiFeedback) {
		...
		CNetEventEntityIDAndInteger ev{ shok::FeedbackEventIds::FEEDBACK_EVENT_ENTITY_HURT, EntityId, damage };
		EGUIX::FeedbackEventHandler::GlobalObj()->FireEvent(&ev);
	}
	if (attacker) {
		for (auto& a : ObserverEntities.ForKeys(shok::AttachmentType::GUARD_GUARDED)) {
			CEvent1Entity ev{ shok::EventIDs::Leader_OnGuardedAttackedBy, attacker->EntityId };
			GetEntityByID(a.second.EntityId)->FireEvent(&ev);
		}
	}

	if (InvulnerabilityFlag)
		damage = 0;

	std::vector<shok::EntityId> idsKilled{};
	int xpToAdd = 0;
	CGLEEntity* firstToDie = this;
	int damageDone = 0;
	if (EventIsBattleOrAutocannon() && !EventIsSerfOrWorker() && damage > 0) { // has potentially soldiers
		CGLEEntity* attackedLeader = this;
		if (EventIsSoldier()) {
			shok::EntityId id = this->GetFirstAttachedToMe(shok::AttachmentType::LEADER_SOLDIER);
			if (id != static_cast<shok::EntityId>(0))
				attackedLeader = GetEntityByID(id);
			if (attackedLeader && attackedLeader->InvulnerabilityFlag)
				damage = 0;
		}
		auto* lBeh = attackedLeader->GetBehaviorDynamic<GGL::CLeaderBehavior>();
		...
		if (lBeh) {
			...
			for (auto& _ : attackedLeader->ObservedEntities.ForKeys(shok::AttachmentType::LEADER_SOLDIER)) {
				currentSol++;
			}
	...
	if (xp && attacker && xpToAdd) {
		auto* al = attacker->GetBehaviorDynamic<GGL::CLeaderBehavior>();
		GGL::CEventEntityIndex xpEv{ shok::EventIDs::CppL_AffectedExperienceGained, attacker->EntityId, xpToAdd };
		for (const auto& at : attacker->ObserverEntities.ForKeys(shok::AttachmentType::HERO_AFFECTED)) {
			GetEntityByID(at.second.EntityId)->FireEvent(&xpEv);
		}
		xpToAdd = xpEv.Index;
		if (al) {
	...
	if (dynamic_cast<GGL::CBuilding*>(this)) {
		if (addStat) {
			if (attackerPlayer != static_cast<shok::PlayerId>(0))
				(*GGL::CGLGameLogic::GlobalObj)->GetPlayer(attackerPlayer)->Statistics.NumberOfBuildingsDestroyed += static_cast<int>(idsKilled.size());
			if (GGL::CPlayerStatus* ps = (*GGL::CGLGameLogic::GlobalObj)->GetPlayer(this->PlayerId))
				ps->Statistics.NumberOfBuildingsLost += static_cast<int>(idsKilled.size());
		}
		...
		if (addStat) {
			if (attackerPlayer != static_cast<shok::PlayerId>(0))
				(*GGL::CGLGameLogic::GlobalObj)->GetPlayer(attackerPlayer)->Statistics.NumberOfUnitsKilled += static_cast<int>(idsKilled.size());
			if (GGL::CPlayerStatus* ps = (*GGL::CGLGameLogic::GlobalObj)->GetPlayer(this->PlayerId))
				ps->Statistics.NumberOfUnitsDied += static_cast<int>(idsKilled.size());
		}
	...
	if (attackerPlayer != static_cast<shok::PlayerId>(0) || HurtEntityCallWithNoAttacker) {
		...
		L.CheckStack(static_cast<int>(idsKilled.size()));
		for (auto i : idsKilled) {
			L.Push(i);
		}
		L.PCall(3 + static_cast<int>(idsKilled.size()), 0, 0);
		L.SetTop(t);
	}
}

void __thiscall EGL::CGLEEntity::AdvancedHurtEntityByStatic(EGL::CGLEEntity *th, EGL::CGLEEntity *attacker,
	int damage, shok::PlayerId attackerFallback, bool uiFeedback, bool xp, bool addStat,
	shok::AdvancedDealDamageSource sourceInfo) {
...
int __thiscall EGL::CGLEEntity::GetMaxHPOverride()
{
	...
	if (EGL::CGLEEntity::UseMaxHPTechBoni)
	{
		...
		if (dynamic_cast<GGL::CSettler*>(this)) {
			for (auto t : static_cast<GGL::CGLSettlerProps*>(et->LogicProps)->ModifyHitpoints.TechList) { // NOLINT(*-pro-type-static-cast-downcast)
...
int __thiscall EGL::CGLEEntity::GetMaxHPOverrideStatic(CGLEEntity* th) {
	return th->GetMaxHPOverride();
}

void __thiscall EGL::CGLEEntity::OnCreateFixHP(CGLEEntity* th, const EGL::CGLEEntityCreator* c)
{
	if (c->Health != -1)
		return;
	...
	else if (auto* b = dynamic_cast<GGL::CBuilding*>(th)) {
		th->Health = th->GetMaxHealth();
		if (!b->IsConstructionFinished()) {
			th->Health = static_cast<int>(static_cast<float>(th->Health) * (*GGL::CLogicProperties::GlobalObj)->ConstructionSiteHealthFactor);
		}
	}
}
...
void EGL::CGLEEntity::HookMaxHP()
{
	...
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x57B798), CppLogic::Hooks::MemberFuncPointerToVoid(&CGLEEntity::GetMaxHPOverride, 0), reinterpret_cast<void*>(0x57B7AD));
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x571B93), &OnCreateFixHPASM, reinterpret_cast<void*>(0x571B98));
...
void EGL::CGLEEntity::HookLuaTaskList()
{
	...
	CppLogic::Hooks::RedirectCall(reinterpret_cast<void*>(0x57D6CA), CppLogic::Hooks::MemberFuncPointerToVoid(&CGLEEntity::AddHandlerLuaTask, 0));
}
...
void __thiscall EGL::CMovingEntity::BuildOnSetPosFixed(CMovingEntity* th)
{
	if (th->MovementState == shok::MovementState::Moving && EGL::CGLEEntity::BuildOnSetPosFixMovement) {
		auto* mov = th->GetBehaviorDynamic<GGL::CBehaviorDefaultMovement>();
		if (mov) {
			movementbeh_setmovetarget(mov, &th->TargetPosition);
		}
	}
	else {
		BB::CEvent ev{ 0x2000D };
		th->FireEvent(&ev);
		if (th->MovementState == shok::MovementState::Moving) {
			ev.EventTypeId = 0x11017;
			th->FireEvent(&ev);
			th->MovementState = shok::MovementState::Moving; // ?? original code
		}
	}
}
...
void EGL::CGLEEntity::HookBuildOnSetPos()
{
	...
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x4ADB16), &CMovingEntity::BuildOnSetPosFixedASM, reinterpret_cast<void*>(0x4ADB76));
}
...
int __stdcall EGL::CGLEEntity::CanCancelStateAdditionalCheck(CGLEEntity* th)
{
	if (th->Health <= 0)
		return 1;
	for (auto a : EGL::CGLEEntity::AdditionalCancelableStates)
		if (a == th->CurrentState)
			return 1;
	return 0;
}
...
void EGL::CGLEEntity::HookCanCancelState()
{
	...
	CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x57A683), &CanCancelStateAdditionalCheckASM, reinterpret_cast<void*>(0x57A689));
}
...
EGL::CGLEEntity* EGL::CGLEEntity::ReplaceEntityWithResourceEntity(EGL::CGLEEntity* e)
{
	...
	if (typeid(*ty) != typeid(GGL::CEntityProperties))
		return nullptr;
	auto* t = static_cast<GGL::CEntityProperties*>(ty); // NOLINT(*-pro-type-static-cast-downcast)
...
void EGL::CGLEEntity::PerformHeal(int r, bool healSoldiers)
{
	...
	if (r <= 0)
		return;
	auto* lb = GetBehavior<GGL::CLeaderBehavior>();
	if (lb && healSoldiers) {
