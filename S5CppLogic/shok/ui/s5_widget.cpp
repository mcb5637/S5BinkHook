...
void EGUIX::CTextButtonWidget::HookFixTextRender()
{
    ...
    CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x55F14F), &CenterRenderAreaASM, reinterpret_cast<void*>(0x55F15D));
...
void EGUIX::CTextButtonWidget::CenterRenderArea(const CTextButtonWidget* th, Rect* render)
{
    if (th->CppLogic_CenterText) {
...
void GGUI::COnScreenElementType::HookDisplayName()
{
    ...
    CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x53F8F1), &GetDisplayNameASM, reinterpret_cast<void*>(0x53F8F9));
}
...
void __stdcall GGUI::C3DOnScreenInformationCustomWidget::RenderHooked(C3DOnScreenInformationCustomWidget* th, shok::Position* screenPos, GGL::IGLGUIInterface::UIData* data, bool* active)
{
    bool skip = false;
    if (th->ShowAllInformationFlag && data->Settler)
        *active = true;
    if (data->Entity) {
        for (auto* b : data->Entity->DisplayBehaviors) {
            if (auto* rb = b->CastToIdentifier<CppLogic::Mod::OnScreenInfoDisplayBehavior>()) {
                skip |= rb->RenderUI(&th->Renderer, screenPos, data, active);
            }
        }
    }
    if (!skip) {
        th->Renderer.RenderInactive(screenPos, data);
        if (*active)
            th->Renderer.RenderActive(screenPos, data);
    }
}
...
void GGUI::C3DOnScreenInformationCustomWidget::HookOnScreenInfoDisplayBehavior()
{
    ...
    CppLogic::Hooks::WriteJump(reinterpret_cast<void*>(0x5367D0), &RenderHookedASM, reinterpret_cast<void*>(0x5367D7));
}
